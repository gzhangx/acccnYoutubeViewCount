...
const sheetsApi = require('./getSheet');
...
function getGoogleLiveApiView(id, key=credentials.youtubeKey) {
    const url = `https://www.googleapis.com/youtube/v3/videos?part=liveStreamingDetails&id=${id}&key=${key}`;
    return request.get(url).send().then(res=>{
        ...
        return mapValues(picked, parseVal);
    });
}
